...
describe("Test the deleteWorkoutExerciseRep mutation", () => {
  ...
  test("It should perform a editWorkoutRep mutation", async () => {
    await supertest(app)
      .post("/graphql")
      .send({
        query: `
      ...
      .expect((res) => {
        ...
        expect(Object.keys(body.data)[0]).toBe("editWorkoutRep");
        expect(Object.keys(body.data.editWorkoutRep)[0]).toBe("id");
        expect(body).toEqual({
          data: {
            editWorkoutRep: {
              id: workout1.id,
